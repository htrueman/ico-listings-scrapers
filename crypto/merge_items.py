...
def init():
    ...
    return imported_members, organizations_file_name, members_file_name
...
def make_merge():
    imported_members, organizations_file_name, members_file_name = init()
                            ...
                            for content_key, content_value in ndo_organization.items():
                                if 'link' in content_key.lower():
                                    if organization[content_key] and \
                                        ...
                                        member = find_related_member(
                                            imported_members_json,
                                            organization['Name'],
                                            ndo_organization['Name'])
                                        imported_members_json.remove(member)
                                        clear_imported_members_json.append(member)
